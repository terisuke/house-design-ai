...
async def process_grid(grid_data: GridData):
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"Error processing grid: {str(e)}"})
...
async def convert_to_2d(file: UploadFile = File(...)):
    ...
    if not file.filename.endswith(".fcstd"):
        return JSONResponse(status_code=400, content={"error": "Invalid file format. Only .fcstd files are supported."})
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"Error converting to 2D: {str(e)}"})
